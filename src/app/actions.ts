...
import { generateYolo as generateYoloFlow } from "@/ai/flows/generate-yolo-flow";
import { Itinerary, ItineraryRequest, ItineraryRequestSchema, Surprise, Yolo } from "@/ai/schemas";
...
export async function generateYolo(): Promise<{ success?: Yolo, error?: { title: string, message: string }}> {
  try {
    const currentHour = new Date().getHours();
    let timeOfDay: 'morning' | 'afternoon' | 'evening';
    if (currentHour >= 5 && currentHour < 12) timeOfDay = 'morning';
    ...
    if (availableVenues.length === 0) {
      throw new Error('No available venues for the current time of day for a YOLO move.');
    }

    const response = await generateYoloFlow({ timeOfDay, availableVenues });
    
    if (!response || !response.name) {
      throw new Error('AI did not return a valid YOLO stop.');
    }
    ...
    const errorMessage = error.message.includes('GEMINI_API_KEY') 
      ? 'Please set the GEMINI_API_KEY environment variable. For more details see https://genkit.dev/docs/plugins/google-genai/'
      : error.message || 'Sorry, I couldn\'t come up with a YOLO move right now. Please try again later.';
    return { error: { title: 'YOLO Failed', message: errorMessage } };
  }
}
